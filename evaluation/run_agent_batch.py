"""Run live agent across all cases and save reproducible artifacts."""
...
OUTPUT_ROOT = Path(__file__).resolve().parent / "results" / "agent"
...
async def main() -> int:
    ...
    for case_id in list_case_ids():
        ...
        agent_meta = result.metadata.get("agent")
        if agent_meta:
            (case_dir / "agent_meta.json").write_text(
    ...
    scored = [r.total_score for r in results if r.error is None]
    summary = {
        "run_id": run_id,
        "case_count": len(results),
        "parse_failures": sum(1 for r in results if r.error is not None),
        "mean_score": statistics.mean(scored) if scored else 0,
        "median_score": statistics.median(scored) if scored else 0,
        "min_score": min(scored) if scored else 0,
        "max_score": max(scored) if scored else 0,
        "average_runtime_ms": statistics.mean([r.runtime_ms for r in results]) if results else 0,
        "total_llm_calls": sum(
            (r.metadata.get("agent") or {}).get("llm_calls", 0) for r in results
        ),
        "retries": sum(
            1 for r in results if (r.metadata.get("agent") or {}).get("retry_occurred")
        ),
    }
